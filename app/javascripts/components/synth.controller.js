...
['$scope', 'MidiDeviceFactory', 'SynthFactory', '$window', '$timeout', '$q', '$interval',
function ($scope, Devices, SynthFactory, $window, $timeout, $q, $interval) {
//TODO: see how many watchers are on page
    ...
    $scope.keyReleased = function(event) {
        wasLastKeyReleased().then(function(wasNotlastKey){
          $scope.keyCurrentlyPressed = wasNotlastKey
        });
    ...
    $scope.requestMidiDeviceAccess = function(event) {
          ...
          .then(function(access) {
              if('function' === typeof access.inputs) {
                  ...
                  if(access.inputs && access.inputs.size > 0) {
                      ...
                      first.value.onmidimessage = onMidiMessage
